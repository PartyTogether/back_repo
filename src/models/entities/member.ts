...
export class Member{
    ...
    @Column({name:'member_bot'})
    bot!: boolean;           // 디스코드 봇 계정 여부

    @Column({name:'member_mfg_enabled'})
    mfg_enabled!: boolean;   // 디스코드 2단계 인증 여부
    ...
    @Column({name:'member_premium_type',type: 'tinyint', default:0})
    premium_type!:number;    // 디스코드 니트로 구독 타입
    ...
    @Column({name:'member_offer_comment'})
